...
resource "azurerm_virtual_machine_extension" "FSLogixConfig" {
  ...
  settings = jsonencode({
    ...
    "commandToExecute" : "powershell -ExecutionPolicy Unrestricted -File InstallFSLogixApps.ps1 -storageAccountName ${azurerm_storage_account.FSLogixStorageAccount.name} -fileShareName ${azurerm_storage_share.AVDProfileShare.name} -secret ${var.StorageAccAccesskeys}"
  })
